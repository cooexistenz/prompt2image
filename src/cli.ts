...
import { isProfileName, renderPrompt, type RenderPromptOptions } from './core/index.js';
...
for (let i = 0; i < args.length; i++) {
  ...
  switch (a) {
    ...
    case '--profile': {
      ...
      if (!isProfileName(p)) fail(`unknown profile "${p}"`);
      opts.profile = p;
      break;
    }
    case '--stdin':
...
if (!promptText.trim()) fail('no prompt given (pass text as an argument or use --stdin)');
